...
namespace Src.OLDPVE.MoveSearchers.TraitsEvaluators
{
    public class UnitsStructureCalculator : IUnitsStructureCalculator
    {
            ...
            CellState freeState, int depth = 1)
        {
            ...
            if (friendlyPositions.Count == 0) return 0;
        ...
        private float GetStructureForUnitOnPosition(Vector2Int position, CellState[,] inputMatrix, CellState unitState,
            CellState baseState, CellState freeState, int depth = 1)
        {
            if (depth == 1)
            {
