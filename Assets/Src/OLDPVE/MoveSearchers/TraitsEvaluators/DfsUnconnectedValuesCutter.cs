...
namespace Src.OLDPVE.MoveSearchers.TraitsEvaluators
{
    public class DfsUnconnectedValuesCutter
    {
        ...
        public CellState[,] CutUnconnectedUnits(CellState[,] inputMatrix, CellState unitState, CellState baseState,
            CellState freeState)
        {
        ...
        private void MarkUnknownCellsAsConnected(CellState[,] matrixCopy, CellState unitState, CellState baseState,
            CellState freeState)
        {
            for (var i = 0; i < matrixCopy.GetLength(0); i++)
            for (var j = 0; j < matrixCopy.GetLength(1); j++)
                if (!matrixCopy[i, j].Equals(unitState) && !matrixCopy[i, j].Equals(baseState) &&
                    !matrixCopy[i, j].Equals(freeState))
        ...
        private void MarkConnectedCells(int basePositionX, int basePositionY, CellState[,] inputMatrix,
            CellState baseState, CellState unitState)
        {
            _connectedCells[basePositionX, basePositionY] = true;
            for (var i = -1; i <= 1; i++)
            for (var j = -1; j <= 1; j++)
            {
                var nextX = basePositionX + i;
                var nextY = basePositionY + j;
                var nextCellDoesNotExist = nextX < 0 || nextX >= inputMatrix.GetLength(0) || nextY < 0 ||
                                           nextY >= inputMatrix.GetLength(1);
