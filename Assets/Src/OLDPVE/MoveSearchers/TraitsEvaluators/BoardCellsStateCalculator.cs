...
namespace Src.OLDPVE.MoveSearchers.TraitsEvaluators
{
    public class BoardCellsStateCalculator : IBoardCellsStateCalculator
    {
        ...
        public CellState[,] GetCurrentBoardState(Player player)
        {
            ...
            for (var j = 0; j < _board.GetLength(1); j++)
            {
                ...
                else if (cell.HasContent(c => c is IPlayerOwned playerOwned && playerOwned.GetOwner() == player))
                {
                    ...
                    if (cell.HasContent(c => c is CastleGO)) boardState[i, j] = CellState.FriendlyBase;
                }
                else
                {
                    boardState[i, j] = CellState.Enemy;
                    if (cell.HasContent(c => c is CastleGO)) boardState[i, j] = CellState.EnemyBase;
                }
            }
        ...
        public CellState[,] GetBoardStateAfterPlayerMove(AbstractMove move)
        {
            ...
            var boardStateWithCutUnconnected =
                _unconnectedValuesCutter.CutUnconnectedUnits(boardState, CellState.Enemy, CellState.EnemyBase,
                    CellState.Free);
